...
const CategoryButton = ({ location, category, className, visible }: Props): JSX.Element => {
   const { language } = useLanguage();
   if (visible === undefined) visible = true; // Default to true if not provided
   
   const categoryDetails = categoriesList.find(cat => cat.en === category || cat.pl === category);
   ...
   return (
      <div
         className={`categoryButton ${className || ''} ${location ? `categoryButton--${location}` : ''} ${visible ? "visible" : "not-visible"}`}
      >
         <img
            className="categoryButton__icon"
            src={require(`../../assets/${categoryNameInPolish}/icon--${categoryNameInPolish}.png`)}
            alt={`${textContent.buttonText.selectCategory[language]}: ${capitalizeFirstLetter(
         ...
         <img
            ...
            alt=""
         />
         <span className="categoryButton__text">{capitalizeFirstLetter(categoryNameInCurrentLanguage)}</span>
      </div>
   );
};
